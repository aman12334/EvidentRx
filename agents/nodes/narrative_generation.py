"""
narrative_generation node — runs ComplianceNarrativeAgent and checkpoints.
"""
...
def narrative_generation(state: InvestigationState, config: RunnableConfig) -> dict:
    session     = config["configurable"]["session"]
    agent       = config["configurable"]["agents"]["narrative_generation"]
    ...
    update = agent.invoke(state, session, wf_memory, case_memory, workflow_step=5)
    ...
    _checkpoint_manager.save(
        ...
        checkpoint_name="narrative_generation",
        state=merged,
    )

    return {**update, "current_node": "narrative_generation"}
