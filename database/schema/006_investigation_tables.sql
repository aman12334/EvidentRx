-- =============================================================================
-- Phase 4: Investigation & Case Orchestration Infrastructure
-- Depends on: 004_audit_tables.sql (investigation_cases, audit_findings)
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. investigation_case_findings — explicit junction with provenance metadata
-- ---------------------------------------------------------------------------
...
CREATE INDEX ix_icf_case_id    ON audit.investigation_case_findings (case_id);
CREATE INDEX ix_icf_finding_id ON audit.investigation_case_findings (finding_id);


-- ---------------------------------------------------------------------------
-- 2. investigation_timelines — append-only, immutable event log per case
--    sequence_number provides total order within a case when timestamps collide.
-- ---------------------------------------------------------------------------
...
CREATE INDEX ix_it_case_id     ON audit.investigation_timelines (case_id, occurred_at);
CREATE INDEX ix_it_event_type  ON audit.investigation_timelines (event_type);
...
-- ---------------------------------------------------------------------------
-- 3. agent_runs — one row per agent invocation (placeholder for LangGraph)
-- ---------------------------------------------------------------------------
...
CREATE INDEX ix_ar_case_id        ON audit.agent_runs (case_id);
CREATE INDEX ix_ar_workflow_run   ON audit.agent_runs (workflow_run_id);
CREATE INDEX ix_ar_status         ON audit.agent_runs (status) WHERE status IN ('pending','running');


-- ---------------------------------------------------------------------------
-- 4. case_risk_snapshots — immutable point-in-time aggregation per case
-- ---------------------------------------------------------------------------
CREATE TABLE audit.case_risk_snapshots (
    ...
    CONSTRAINT ck_snapshot_trigger
        CHECK (snapshot_trigger IN ('case_created','finding_added','manual','scheduled','status_changed'))
);

CREATE INDEX ix_crs_case_id ON audit.case_risk_snapshots (case_id, snapshot_at DESC);
...
-- ---------------------------------------------------------------------------
-- 5. workflow_checkpoints — serialized LangGraph state for pause/resume
-- ---------------------------------------------------------------------------
...
CREATE INDEX ix_wc_case_id      ON audit.workflow_checkpoints (case_id);
